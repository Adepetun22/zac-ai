...
export default function AIModelsPage() {
  ...
  const { 
    ...
    fetchAiModels,
    clearError,
    generateAIResponse  // Added the new AI function
  } = useDashboardStore();
  ...
  const handleRun = async (model) => {
    try {
      // Example: Run a simple test with the model
      const response = await generateAIResponse("Say hello in a professional manner", model.id);
      showToast(`Test response from ${model.name}: ${response.substring(0, 50)}...`);
    } catch (err) {
      showToast(`Error running ${model.name}: ${err.message}`);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <button 
          onClick={async () => {
            // Example of adding a new model with test functionality
            try {
              const response = await generateAIResponse("Introduce yourself as an AI assistant", "gpt-4o");
              const newModel = {
              ...
              addAiModel(newModel);
              showToast('New model added with test response');
            } catch (err) {
              showToast('Error creating model: ' + err.message);
            }
          }}
