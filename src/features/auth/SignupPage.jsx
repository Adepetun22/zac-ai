        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center gap-3">
              ...
              <span className="text-sm font-medium text-indigo-600 dark:text-indigo-400">Get Started</span>
              <ThemeToggle />
            </div>
        ...
        {isMenuOpen && (
          <div className="md:hidden bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-200 dark:border-slate-700">
            <div className="px-4 py-4 space-y-3">
              ...
              <Link to="/login" onClick={() => setIsMenuOpen(false)} className="block text-sm font-medium text-slate-600 dark:text-[var(--color-text-primary)] hover:text-indigo-600 py-2">Sign in</Link>
      ...
      <main className="flex-1 flex items-center justify-center px-4 pt-24 pb-16">
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            ...
            <p className="text-slate-500 dark:text-[var(--color-text-secondary)]">Sign up to get started with Zac-AI Dashboard</p>
          </div>
          ...
          <div className="bg-white/80 dark:bg-[var(--color-bg-surface)]/80 backdrop-blur-sm rounded-2xl shadow-xl border border-slate-200 dark:border-[var(--color-border-subtle)] p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label htmlFor="name" className="block text-sm font-medium text-slate-700 dark:text-[var(--color-text-secondary)] mb-1.5">Full Name</label>
                <input id="name" type="text" value={name} onChange={e => setName(e.target.value)} className={inputCls} placeholder="Enter your full name" required />
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input id="password" type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} className={inputCls + ' pr-12'} placeholder="Create a password" required minLength={6} />
              ...
              <div>
                <label htmlFor="confirm-password" className="block text-sm font-medium text-slate-700 dark:text-[var(--color-text-secondary)] mb-1.5">Confirm Password</label>
                <div className="relative">
                  <input id="confirm-password" type={showConfirmPassword ? 'text' : 'password'} value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} className={inputCls + ' pr-12'} placeholder="Confirm your password" required />
              ...
              <label className="flex items-start gap-2 text-sm text-slate-600 dark:text-[var(--color-text-secondary)] cursor-pointer">
